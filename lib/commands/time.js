...
module.exports = {
  ...
  getResponse: function(args, store, cb) {
    ...
    if (args[0] === 'remove' && parseInt(args[1])) {
      return removeTime(store, parseInt(args[1]), cb);
    }
    if (args[0] === 'clear') {
...
function listTimes(store, cb) {
  ...
  list.then(function(times) {
    ...
    cb(result + '</ol>', {color : 'gray'});
  });
}

function removeTime(store, pos, cb) {
  var list = store.get(LIST_KEY);
  list.then(function(times) {
    if (times && pos > 0 && pos <= times.length) {
      times.sort();
      ...
      cb("Could not remove a time from the list");
    }
  });
}

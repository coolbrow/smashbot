...
function listTimes(store, send) {
  return function *() {
    ...
    if (times) {
      ...
      var now = getNow();
      var ppMilli = postpone.minutes * 60 * 1000;
      var diffTime = new Date(new Date(postpone.time) - now + ppMilli);
  ...
  return function *() {
    if (!minutes) {
      ...
      if (minutes > 0) {
        ...
        store.set(POSTPONE_KEY, {});
        yield send("The " + formatTime(nextTime) + " match is no longer postponed");
      }
    }
  }
}
